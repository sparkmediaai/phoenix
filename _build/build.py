...
# The logo lockup, inline so PHOENIX (fill="currentColor") takes the header's text colour.
# Generated by _tools/make_logo.py; edit the paths there, not in assets/logo.svg.
LOGO = (open(os.path.join(ROOT, "assets", "logo.svg"), encoding="utf-8").read().strip()
        .replace('role="img" aria-label="Phoenix Solutions"', 'aria-hidden="true" focusable="false"', 1))
